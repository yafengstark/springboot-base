...
//        MyWebSocketService.sendMessageTo(JSONObject.toJSONString(user),"13888888888");
    ...
    public User updateForgetPasswd(JSONObject requestJson) throws Exception {
        ...
        // 修改密码
        List<SmsVerify> smsVerifies = smsVerifyService.getByMobileAndCaptchaAndType(
                mobile,
    ...
    public User insertUserByAdmin(JSONObject requestJson) throws Exception {
        ...
        this.insert(user);// 插入用户
        // 使用builder
        infoToUserService.insert(InfoToUser.builder()
                .userNo(userNo)
                .identityInfo(user.getUsername())
                .identityType(Constant.LOGIN_USERNAME).build()
        );
        UserToRole userToRole = UserToRole.builder()
                .userNo(user.getUserNo())
                .roleCode(role.getRoleCode()).build();
        userToRoleService.insert(userToRole);
        return user;
    }
