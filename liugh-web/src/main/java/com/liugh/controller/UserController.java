...
public class UserController {
    ...
    public ResponseModel<User> getUser(@ApiIgnore @CurrentUser User user) throws Exception {
        return ResponseHelper.buildResponseModel(user);
    }

    @ApiOperation(value = "重新设置移动电话", notes = "")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "body",
                    name = "requestJson",
                    dataType = "String",
                    value = "{\"captcha\":\"\",\"newMobile\":\"\"}",
                    required = true )
    })
    @PostMapping("/mobile")
    public ResponseModel<String> resetMobile(@ApiIgnore @CurrentUser User currentUser,
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "requestJson",
                    value = "{\"oldPassword\":\"13888888888\",\"password\":\"123456\",\"rePassword\":\"123456\"}"
                    , required = true,
                    dataType = "String",
                    paramType = "body")
    })
    @PostMapping("/password")
    public ResponseModel<String> resetPassWord(@ApiIgnore@CurrentUser User currentUser,
     ...
     * @throws Exception
     */
    @ApiOperation(value = "管理端修改某用户密码", notes = "")
    @ApiImplicitParams({
            // 内含参数
            @ApiImplicitParam(name = "requestJson",
                    value = "{\"userNo\":\"13888888888\",\"password\":\"123456\",\"rePassword\":\"123456\"}"
                    , required = true,
                    dataType = "String",
                    paramType = "body")
    })
    ...
    @ApiOperation(value = "修改当前用户信息", notes = "")
    @ApiImplicitParams({
            // 内含参数
            @ApiImplicitParam(name = "requestJson",
                    value = "{\"username\":\"13888888888\"," +
                            "\"avatar\":\"123456\"," +
                            "\"email\":\"123456\"," +
                            "\"job\":\"\"}"
                    , required = true,
    ...
    @PostMapping("/info")
    public ResponseModel<User> resetUserInfo(@ApiIgnore@CurrentUser User currentUser,
                                             @RequestBody JSONObject requestJson) throws Exception {
